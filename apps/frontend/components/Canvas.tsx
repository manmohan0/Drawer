...
export const Canvas = ({ roomId, ws }: { roomId: string; ws: WebSocket }) => {
  ...
  const handlePromptSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res && res.data.success) {
        ...
        const shapesArray = Array.isArray(shapesData)
          ? shapesData
          : shapesData?.shapes || [];
  ...
  const handleRemoveUser = async (targetUserId: string) => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${BACKEND_URL}/room/${currentRoom.slug}/removeUser`,
        {
          headers: {
            Authorization: token,
          },
      ...
      case "rectangle":
        return {
          ...
          metrics: [
            {
              label: "Position X",
              value: Math.round(selectedShape.startX),
              key: "startX",
              editable: true,
            },
            {
              label: "Position Y",
              value: Math.round(selectedShape.startY),
              key: "startY",
              editable: true,
            },
            {
            ...
            {
              label: "Area",
              value: `${Math.round(Math.abs(selectedShape.width * selectedShape.height))} px²`,
              editable: false,
            },
          ],
        };
      case "circle":
        ...
        return {
          ...
          metrics: [
            {
              label: "Center X",
              value: Math.round(selectedShape.centerX),
              key: "centerX",
              editable: true,
            },
            {
              label: "Center Y",
              value: Math.round(selectedShape.centerY),
              key: "centerY",
              editable: true,
            },
            ...
            {
              label: "Circumference",
              value: `${Math.round(2 * Math.PI * radius)} px`,
              editable: false,
            },
      ...
      case "line":
        ...
        return {
          ...
          metrics: [
            ...
            {
              label: "End X",
              value: Math.round(selectedShape.endX),
              key: "endX",
              editable: true,
            },
            {
              label: "End Y",
              value: Math.round(selectedShape.endY),
              key: "endY",
              editable: true,
            },
      ...
      case "image":
        return {
          ...
          metrics: [
            ...
            {
              label: "URL Status",
              value: selectedShape.url ? "Loaded" : "No image uploaded",
              editable: false,
            },
          ],
        };
  ...
  const handleMetricChange = (key: string, valStr: string) => {
    ...
    metricChangeTimeoutRef.current = setTimeout(() => {
      ...
      if (key === "text") {
        ...
        ["startX", "startY", "centerX", "centerY", "endX", "endY"].includes(key)
      ) {
        ...
        if (updatedShape.type === "line") {
          ...
          const fromX =
            (origShape as any).startX !== undefined
              ? (origShape as any).startX
              : (origShape as any).centerX;
          const fromY =
            (origShape as any).startY !== undefined
              ? (origShape as any).startY
              : (origShape as any).centerY;
          const toX =
            (updatedShape as any).startX !== undefined
              ? (updatedShape as any).startX
              : (updatedShape as any).centerX;
          const toY =
            (updatedShape as any).startY !== undefined
              ? (updatedShape as any).startY
              : (updatedShape as any).centerY;
          extraFields = {
            fromX,
            fromY,
            toX,
  ...
  const handleColorChange = (key: "color" | "bg_color", newColor: string) => {
    ...
    const eventType =
      key === "color" ? EventType.CHANGE_STROKE : EventType.CHANGE_FILL;
    const fromColor =
      (selectedShape as any)[key] || eventType === EventType.CHANGE_STROKE
        ? "#000000"
        : "";
    ...
    colorChangeTimeoutRef.current = setTimeout(
      () =>
        game.updateShape(updatedShape, eventType, {
          fromColor,
          toColor,
        }),
      500,
  ...
  const creatorName = selectedShape
    ? game?.getUserName(selectedShape.userId)
    : null;
  const updaterName = selectedShape
    ? game?.getUserName(selectedShape.updatedByUserId)
    : null;
    ...
    <div className="relative overflow-hidden w-screen h-screen">
      ...
      {details && (
        <div className="absolute top-4 left-4 z-50 w-72 bg-zinc-900/75 backdrop-blur-md border border-zinc-800 shadow-2xl rounded-2xl overflow-hidden transition-all duration-300 transform scale-100 hover:shadow-orange-500/10 animate-in fade-in duration-200">
          {/* Header */}
          <div
            className={`flex items-center justify-between px-4 py-3 bg-zinc-950/50 select-none ${!isShapeDetailsCollapsed ? "border-b border-zinc-800" : ""}`}
          >
            <div className="flex items-center space-x-2">
              {details.icon}
              <span className="font-semibold text-zinc-200 tracking-wide text-sm">
                {details.title}
              </span>
            </div>
            <div className="flex items-center space-x-2">
              ...
              <button
                onClick={() =>
                  setIsShapeDetailsCollapsed(!isShapeDetailsCollapsed)
                }
              ...
              >
                <ChevronDown
                  className={`w-3.5 h-3.5 transition-transform duration-200 ${isShapeDetailsCollapsed ? "" : "rotate-180"}`}
                />
              </button>
            </div>
          </div>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {details.metrics.map((metric, idx) => (
                  <div
                    key={idx}
                    className={`flex flex-col p-2 rounded-xl bg-zinc-950/40 border border-zinc-850 transition-all duration-200 hover:bg-zinc-950 hover:border-orange-500/30 hover:shadow-sm ${
                      metric.label === "Area" ||
                      metric.label === "Length" ||
                      metric.label === "Circumference" ||
                      metric.label === "URL Status" ||
                      metric.key === "text"
                        ? "col-span-2"
                        : ""
                    }`}
                ...
                (selectedShape.type === "rectangle" ||
                  ...
                  <div className="pt-3 border-t border-zinc-850 space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div
                        className={`relative flex items-center justify-center w-8 h-8 rounded-full border border-zinc-800 bg-zinc-950 hover:bg-zinc-900 transition-colors shadow-sm ${myRole === "Viewer" ? "cursor-not-allowed opacity-50" : "cursor-pointer group"}`}
                      >
                        <input
                          type="color"
                        ...
                        <div
                          ...
                          style={{
                            backgroundColor: selectedShape.color || "#000000",
                          }}
                        />
                      </div>
                    </div>
      ...
      {/* Profile & Replay menu */}
      <div
        className="absolute top-4 right-4 z-50 flex items-center space-x-2.5"
        ref={dropdownRef}
      >
        {!isReplayMode && (
          <button
                ...
                <div className="grid grid-cols-2 gap-2 text-xs text-zinc-400">
                  ...
                  <div className="flex flex-col bg-zinc-950 p-3.5 rounded-xl border border-zinc-850/50 shadow-sm">
                    ...
                    >
                      {Array.isArray(currentRoom.admin)
                        ? currentRoom.admin[0]
                          ? `${currentRoom.admin[0].firstName} ${currentRoom.admin[0].lastName}`
                          : "Unknown"
                        : currentRoom.admin
                          ? `${currentRoom.admin.firstName} ${currentRoom.admin.lastName}`
                          : "Unknown"}
      ...
      {Object.keys(activeUsers).length > 0 && (
        <div
          className={`absolute top-20 right-4 z-50 w-64 bg-zinc-900/80 backdrop-blur-md border border-zinc-800 shadow-lg rounded-2xl p-4 flex flex-col transition-all duration-300 overflow-hidden hover:shadow-orange-500/5 ${isMembersListCollapsed ? "max-h-[52px]" : "max-h-[300px]"}`}
        >
          <div
            className={`flex items-center justify-between ${!isMembersListCollapsed ? "pb-2 border-b border-zinc-800" : ""}`}
          >
            <div className="flex items-center space-x-2">
            ...
            <button
              ...
              title={
                isMembersListCollapsed ? "Expand Members" : "Collapse Members"
              }
            >
              <ChevronDown
                className={`w-3.5 h-3.5 transition-transform duration-200 ${isMembersListCollapsed ? "" : "rotate-180"}`}
              />
            </button>
          </div>
            ...
            <div className="overflow-y-auto mt-2 space-y-1 flex-1 pr-1 scrollbar-thin">
              {Object.entries(activeUsers).map(([uId, u]) => {
                ...
                return (
                  <div
                    key={uId}
                    className="flex items-center justify-between p-2 rounded-xl hover:bg-zinc-950/80 transition-colors"
                  >
                    <div className="flex items-center space-x-2">
                      ...
                      <span
                        className="text-xs text-zinc-200 font-medium truncate max-w-[130px]"
                        title={name}
                      >
                        {name} {isMe && "(You)"}
                      </span>
        ...
        <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-lg px-4 sm:px-0">
          ...
          >
            <input
              ...
              placeholder={
                loading
                  ? "AI is thinking and drawing..."
                  : "Ask AI to draw something... (e.g. 'draw a red circle')"
              }
        ...
        <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-xl px-4 sm:px-0">
          <div className="bg-zinc-900/85 backdrop-blur-xl border border-zinc-800 shadow-[0_20px_50px_rgba(249,115,22,0.08)] rounded-3xl p-5 flex flex-col space-y-4 transition-all duration-300">
            ...
            <div className="flex items-center justify-between">
              ...
              <div
                className="text-xs font-medium text-zinc-400 max-w-[280px] truncate bg-zinc-950 border border-zinc-850 rounded-xl px-3 py-1.5"
                title={replayEvents[replayCurrentIndex]?.description}
              >
                {replayEvents[replayCurrentIndex]?.description ||
                  "Empty room - no events recorded"}
              </div>
            </div>

            {/* Scrubber slider */}
            <div className="flex items-center space-x-3 group">
              <span className="text-[10px] font-bold text-zinc-500 select-none w-8 text-center font-mono">
                START
              </span>
              <input
                type="range"
